...
/*
 * Verify Schnorr-style signature
 */
static cyxwiz_error_t verify_signature(
    ...
    const uint8_t *signature)
{
    ...
    uint8_t sG[32];
    uint8_t eP[32];
    uint8_t R_plus_eP[32];
    crypto_generichash_state state;
    uint8_t hash[64];

    /* Validate R is a valid point */
    ...
    /* eP = e * P */
    if (crypto_scalarmult_ed25519_noclamp(eP, e, public_key) != 0) {
        return CYXWIZ_ERR_CRYPTO;
    }

    /* R + eP */
    if (crypto_core_ed25519_add(R_plus_eP, R, eP) != 0) {
        return CYXWIZ_ERR_CRYPTO;
    }

    /* Verify: sG == R + eP */
    if (sodium_memcmp(sG, R_plus_eP, 32) != 0) {
        return CYXWIZ_ERR_CREDENTIAL_INVALID;
    }

    cyxwiz_secure_zero(e, sizeof(e));
    cyxwiz_secure_zero(sG, sizeof(sG));
    cyxwiz_secure_zero(eP, sizeof(eP));
    cyxwiz_secure_zero(R_plus_eP, sizeof(R_plus_eP));
    cyxwiz_secure_zero(hash, sizeof(hash));
    cyxwiz_secure_zero(&state, sizeof(state));
